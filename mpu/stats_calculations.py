...
AGGS = {"Amount": ["sum", "median"], "PriceXAmount": "sum", "Price": "median"}
...
def aggregate_data(
    ...
    if group_name is None:
        ...
        _stats_series.columns = [
            f"{label}/{agg}" for label, agg in _stats_series.columns
        ]
    ...
    if isinstance(_stats_series, pd.Series):
        ...
        _stats_series.index = pd.MultiIndex.from_product(
            [[index_name], [index_name], _stats_series.index]
        )
    else:
